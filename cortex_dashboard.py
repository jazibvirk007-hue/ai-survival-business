...
def _ceo_state(pending_count):
    """Build the small, non-secret state surface shared with Cortex CEO chat."""
    return {
...
def build_snapshot():
    ...
    state = _ceo_state(pending_count)
    try:
...
def build_chat_state():
    ...
    return {
        ...
        "paid_orders": snapshot["verified_payments"],
        "products": snapshot["orders"],
        "current_decision": snapshot["ceo"]["action"],
...
document.getElementById('chatBtn').addEventListener('click',chat);document.getElementById('chatInput').addEventListener('keydown',e=>{if(e.key==='Enter')chat()});refresh();setInterval(refresh,5000);
</script></body></html>'''
...
class CortexHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if length <= 0 or length > MAX_REQUEST_BYTES:
            self._send(413, json.dumps({"error": "request too large or empty"}), "application/json; charset=utf-8")
            return
        try:
            ...
            provider = provider_from_env()
            reply = CortexCEOChat(provider).respond(build_chat_state(), payload["message"])
            ...
            self._send(503, json.dumps({"error": "CEO provider unavailable"}), "application/json; charset=utf-8")
