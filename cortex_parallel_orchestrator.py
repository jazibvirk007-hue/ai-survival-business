...
class CortexParallelOrchestrator:
    ...
    def decide(self, observations: Dict[str, Any]) -> Dict[str, Any]:
        for name, result in observations.get("observations", {}).items():
            if isinstance(result, dict) and result.get("action"):
                return {"action": result["action"], "workstream": name, "reason": result.get("reason", ""), "transition_budget": 1}
    ...
    def cycle(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"observations": observations, "decision": self.decide(observations), "execution": "decision_only"}

    def status(self) -> Dict[str, Any]:
        return {"version": self.VERSION, "workstreams": sorted(self.workstreams), "max_transitions_per_cycle": 1, "execution": "decision_only", "revenue": "verified_observations_only"}
