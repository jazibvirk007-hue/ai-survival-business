...
echo Checking Cortex startup health...
.venv\Scripts\python.exe -c "from cortex_local_health import assert_local_health; assert_local_health(); print('Cortex local health: READY')"
if errorlevel 1 (
  echo Cortex local health check failed. Startup blocked safely.
  pause
  exit /b 1
)
