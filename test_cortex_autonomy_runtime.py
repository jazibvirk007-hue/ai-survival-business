...
class CortexAutonomyRuntimeTests(unittest.TestCase):
    def test_research_patch_advances_operational_state(self):
        with tempfile.TemporaryDirectory() as directory:
            runtime = CortexAutonomyRuntime({"opportunity": "ai automation"}, state_path=os.path.join(directory, "runtime.json"))
    ...
    def test_runtime_never_allows_financial_truth_to_be_patched(self):
        with tempfile.TemporaryDirectory() as directory:
            runtime = CortexAutonomyRuntime({"verified_revenue": 125, "cash": 50, "recent_action_failed": True}, state_path=os.path.join(directory, "runtime.json"))
    ...
    def test_failed_handler_sets_failure_observation(self):
        with tempfile.TemporaryDirectory() as directory:
            runtime = CortexAutonomyRuntime({"recent_action_failed": True}, state_path=os.path.join(directory, "runtime.json"))
    ...
    def test_snapshot_exposes_real_runtime_state(self):
        with tempfile.TemporaryDirectory() as directory:
            runtime = CortexAutonomyRuntime({"market_researched": True, "verified_revenue": 10}, state_path=os.path.join(directory, "runtime.json"))
